...
switch nargin
	...
	case 1 %If GSINFO is called with 1 argument, 
        % list available experiments in the specified file
		...
		if nargout == 0
            ...
            [~, v_order] = sort(double(string(cell_out(:, 1))), 'ascend');
            cell_out = cell_out(v_order, :);
			if nMethods > 0
...
% 			end
	...
	case 2 % If GSINFO is called with 2 arguments, 
        % show description for the specified experiment function
		className = fixClassName(varargin{1});
		numStr = varargin{2};
